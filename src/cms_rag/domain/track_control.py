...
def parse_track_request(text: str) -> TrackRequest:
    ...
    field_signal = bool(re.search(r"\b(hiz|surat|yon|rota|gemi tipi|iz tipi)\w*\b", normalized))
    ...
    if heading_decimal is not None:
        if not heading_decimal.is_integer():
            return TrackRequest(TrackIntent.AMBIGUOUS, reason="Yön tam sayı derece olmalıdır.")
